...
export const rushHour: Scenario = {
  ...
  durationSec: 10 * H,
  clockStartHour: 7,
  defaultSpeed: 480, // 10h day in ~1.25 wall-minutes
  initialDials: {
    workload: 'agentic-dev', clientTimeoutSec: 120, retryStrategy: 'aggressive',
    numUsers: 0, admissionLimit: 100_000, prefillServers: 6,
  },
  loadCurve: (t) => rushHourUsers(t + 7 * H),
  narrator: [
    { id: 'welcome', text: '7 AM. Cron jobs hum along. Watch the pipeline: requests take a decode slot FIRST, then wait for prefill. Remember that.', when: ({ t }) => t > 60 },
    { id: 'ramp', text: '9 AM. The org logs on. Every request is an agentic-dev monster: prompts up to 1M tokens, and 32K-128K of thinking before anyone sees a word.', when: ({ t }) => t > 2 * H },
    { id: 'oversub', text: '10 AM. Demand now exceeds supply — but goodput is still decent. Are we fine? We are not fine.', when: ({ t }) => t > 3 * H },
    { id: 'tpm', text: 'The TPM counter is enormous and the officials are delighted. Look closer: only tokens that REACH a user count. The rest is very expensive heat.', when: ({ t, sim }) => t > 3.75 * H && rollingGoodputPct(sim.history, 600) < 60 },
    { id: 'collapse', text: 'Nothing is broken. Everything is busy. Goodput is cratering while every GPU reads 100%. Welcome to the worst kind of outage.', when: ({ t, sim }) => t > 3.5 * H && rollingGoodputPct(sim.history, 600) < 25 },
    { id: 'zombies', text: 'Look at the waste band: those are tokens computed for clients that hung up long ago. The queues are full of ghosts — and their retries.', when: ({ t, sim }) => t > 4 * H && rollingGoodputPct(sim.history, 600) < 20 },
    ...
    { id: 'found-balance', text: 'More prefill servers? Did you find the perfect balance without admission limits...Does this also work for "The Spike"? Can we get more total tokens another way?', when: ({ t, sim }) => t > 9.5 * H && sim.dials.prefillServers === 7 },
    ...
    { id: 'gate', text: 'THERE it is. Failing cheap at the front door instead of expensive in the pipeline. Fewer requests in, more tokens out.', when: ({ t, sim }) => t > 3 * H && sim.dials.admissionLimit < 100 },
    ...
    { id: 'exodus', text: '3 PM. People are giving up and going home. In production, this was our fix. That is not a fix.', when: ({ t }) => t > 8 * H },
    { id: 'close', text: 'Day over. Run it again — this time the dials are yours. Hint: the hero dial is the one that says no.', when: ({ t }) => t > 9.5 * H },
  ],
  win: {
    windowStartSec: 2 * H, windowEndSec: 8 * H, // 9 AM-3 PM on a 7 AM clock
